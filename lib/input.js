...
const departmentToDelete = async db => {
    ...
    db.deleteDepartment(promptResult.id);
}
...
const employeeToDelete = async db => {
    ...
    db.deleteEmployee(promptResult.id);
}
...
const managerToView = async db => {
    const promptResult = await inquirer.prompt([
        {
            type: "input",
            message: "What is the ID of the manager who's employees you would like to view?",
            name: "id"
        }
    ]);
